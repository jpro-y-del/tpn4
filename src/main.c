          ...
          if (a==8){
                contrar[8]='\0';
                 if (strcmp(contdf,contrar)==0){
                    ledv= GPIOA-> BSRR|= (1<<8);//ledv y permitir nueva cont 
                    hash= true;
                  }
                  else {
                    ledr= GPIOA-> BSRR|= (1<<9);
...
// leo a traves de la libreria los pines ingresados y si
// son = a la contraseña ya hecha then = 0 y se cambia la cont
//for (i=0;i<8;i++){
//              contdf[i]=contrar[i];
 //         }